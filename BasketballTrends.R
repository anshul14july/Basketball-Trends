...
plot <- function(data,rows=1:10) {
  val <- data[rows,,drop=F]
  matplot(t(val),type = "b",pch = 15:18,col = c(1:4,6))
  legend("bottomleft",legend=Players[rows],inset = 0.01,horiz = F,pch = 15:18, col = c(1:4,6))
}
plot(FreeThrowsAttempts/Games)
plot(FreeThrows/FreeThrowsAttempts)



#playing style
plot((Points-FreeThrows)/FieldGoals)
